...
export default function FontsTab({
  ...
  const handleLetterEmbed = async () => {
    ...
    while (true) {
      ...
      totalErrors += r.errors
      const processed = offset + r.batchSize
      setLetterEmbProgress({ processed, total: r.total, done: totalDone, errors: totalErrors })
      if (r.batchSize < 1 || processed >= r.total) break
      offset += 1
    }
    setLetterEmbStatus('done')
    setTimeout(() => { setLetterEmbStatus('idle'); setLetterEmbProgress(null) }, 10000)
  }
        ...
        <div className="flex flex-col items-end gap-0.5">
          ...
          {letterEmbProgress && letterEmbStatus === 'running' && letterEmbProgress.total > 0 && (
            <div className="w-full overflow-hidden rounded-full" style={{ height: '3px', background: 'rgba(168,85,247,.15)' }}>
              <div
                className="h-full rounded-full transition-all duration-300"
                style={{ width: `${Math.round((letterEmbProgress.processed / letterEmbProgress.total) * 100)}%`, background: '#a855f7' }}
              />
            </div>
          )}
          {letterEmbStatus === 'done' && letterEmbProgress && (
            <p className="text-[11px]" style={{ color: '#7c3aed' }}>
              {letterEmbProgress.done} פונטים{letterEmbProgress.errors > 0 ? ` · ${letterEmbProgress.errors} שגיאות` : ''}
            </p>
          )}
        </div>
