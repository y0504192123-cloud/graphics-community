...
export default async function PrivacyPage() {
  ...
  const { data } = await admin.from('site_settings').select('value').eq('key', 'privacy_policy').single()
  const content = data?.value ?? 'מדיניות הפרטיות טרם הוגדרה.'

  return <PolicyPage title="מדיניות הפרטיות" content={content} />
}
